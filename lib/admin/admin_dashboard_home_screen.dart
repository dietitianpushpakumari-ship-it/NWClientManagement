...
import 'package:nutricare_client_management/admin/database_provider.dart'; // 🎯 Needed for Tenant Check

// 🎯 SCREENS
import 'package:nutricare_client_management/admin/company_list_Screen.dart';
import 'package:nutricare_client_management/admin/migration/migration%20dashboard.dart'; // Ensure filename matches
...
class AdminDashboardHomeScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    // If this is NULL, we are in the MASTER (Primary) Database.
    // If this has data, we are inside a Tenant (Clinic) Database.
          ...
          SafeArea(
            child: adminAsync.when(
              ...
              data: (admin) {
                ...
                // 🎯 2. STRICT PERMISSION LOGIC
                // Super Admin: Must have Role AND be in Master DB (Tenant == null)
                final bool isSuperAdmin = admin.role == AdminRole.superAdmin && currentTenant == null;

                // Clinic Admin: Has Role 'clinicAdmin'
                final bool isClinicAdmin = admin.role == AdminRole.clinicAdmin;

                // View Switcher: Only for Owners/Admins
                ...
                return CustomScrollView(
                  ...
                  slivers: [
                              ...
                              child: Container(
                                ...
                                child: CircleAvatar(
                                  ...
                                  child: admin.photoUrl.isEmpty
                                      ? Text(
                                    admin.firstName.isNotEmpty ? admin.firstName[0] : 'A',
                    ...
                    // 3. CORE ACTIONS GRID
                    // We pass the computed flags down to control visibility
    ...
    return SliverPadding(
      ...
      sliver: SliverGrid.count(
        ...
        children: [
          ...
          // 🛑 SUPER ADMIN ONLY (Restricted to Master DB)
          if (isSuperAdmin) ...[
            _buildBentoAction(
              context,
              "Add Clinic",
          ...
          // 🏥 COMMON ACTIONS (Available to Tenant Admins & Staff based on permission)
