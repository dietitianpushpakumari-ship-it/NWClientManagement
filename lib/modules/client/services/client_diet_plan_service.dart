...
class ClientDietPlanService {
  ...
  // Sub-collection within the client document for tracking assigned plans
  ...
  // 🎯 CRITICAL: Define the collection refere  nce with the converter.
  ...
  Future<void> assignPlanToClient({
    ...
    try {
      ...
      logger.i(
        'Successfully assigned plan ${masterPlan.name} to client $clientId',
      );
    } catch (e, stack) {
  ...
  Future<void> updatePlan(ClientDietPlanModel plan) async {
    ...
    await _clientPlansCollection
        .doc(plan.id)
        .set(plan, SetOptions(merge: true));
  }
  ...
  Stream<List<ClientDietPlanModel>> streamAllNonDeletedPlansForClient(
    String clientId,
  ) {
  ...
  Future<void> unassignPlanFromClient({
    ...
    final querySnapshot = await _firestore
        ...
        .get(); // Get the QuerySnapshot once

    // 2. Check if a document was found.
    if (querySnapshot.docs.isNotEmpty) {
      // 3. Get the DocumentReference of the first (and only) result.
      final docReference = querySnapshot.docs.first.reference;
      await docReference.delete();
    } else {
      // This is expected if the plan was already unassigned.
      print(
        'Warning: Active plan assignment not found for client $clientId and plan $masterPlanId.',
      );
    }
  }

  Stream<bool> checkIfPlanIsAssigned(String clientId, String masterPlanId) {
    return _firestore
        .collection(_assignedPlansSubCollection)
        .where('clientId', isEqualTo: clientId)
        .where('masterPlanId', isEqualTo: masterPlanId)
        .snapshots()
        .map((snapshot) {
          // If the document exists and 'isActive' is true, it is assigned.
          // We check for existence first, then isActive if it's a soft-delete approach.
          // Since we use hard-delete/isActive: true, existence is enough.
          return snapshot.docs.isNotEmpty;
        });
  }

  Stream<List<ClientDietPlanModel>>? streamAssignedPlans(String clientId) {
  ...
  Stream<List<String>> streamAssignedPlanIds(String clientId) {
    return _clientPlansCollection
       ...
       // .where('isActive', isEqualTo: true)
        .snapshots()
        .map((snapshot) {
      // Return a list of masterPlanId strings
