...
class DiseaseMasterListScreen extends ConsumerStatefulWidget {
  const DiseaseMasterListScreen({super.key});

  @override
  ConsumerState<DiseaseMasterListScreen> createState() => _DiseaseMasterListScreenState();
}

class _DiseaseMasterListScreenState extends ConsumerState<DiseaseMasterListScreen> {
          ...
          children: [
            ...
            Expanded(
              child: StreamBuilder<List<DiseaseMasterModel>>(
                stream: ref.watch(diseaseMasterServiceProvider).getActiveDiseases(),
                builder: (context, snapshot) {
                  ...
                  if(filteredList.isEmpty) return const Center(child: Text("No diseases found."));
      ...
      child: Container(
        ...
        child: TextField(
          ...
          decoration: InputDecoration(
            hintText: "Search condition by name...",
  ...
  // 🎯 MODIFIED: Card Style
  Widget _buildDiseaseCard(BuildContext context, DiseaseMasterModel item) {
    return Container(
      decoration: BoxDecoration(
        color: Colors.white,
        borderRadius: BorderRadius.circular(16),
        boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.04), blurRadius: 10, offset: const Offset(0, 4))],
        border: Border(left: BorderSide(color: Colors.deepOrange.withOpacity(0.5), width: 4)),
      ),
      child: ListTile(
        leading: Container(padding: const EdgeInsets.all(10), decoration: BoxDecoration(color: Colors.deepOrange.shade50, borderRadius: BorderRadius.circular(10)), child: const Icon(Icons.coronavirus, color: Colors.deepOrange)),
        ...
        trailing: IconButton(icon: Icon(Icons.edit_outlined, color: Theme.of(context).colorScheme.primary), onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (_) => DiseaseMasterEntryScreen(diseaseToEdit: item)))),
      ),
    );
  }
        ...
        child: Container(
          ...
          child: Row(
            children: [
              ...
              Container(padding: const EdgeInsets.all(10), decoration: BoxDecoration(color: Colors.deepOrange.withOpacity(.1), shape: BoxShape.circle), child: const Icon(Icons.coronavirus, color: Colors.deepOrange)),
            ],
          ),
        ),
      ),
